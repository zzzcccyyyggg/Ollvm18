...
Function *StringEncryptionPass::buildDecryptFunction(
    ...
    FunctionType *FuncTy = FunctionType::get(
        Type::getVoidTy(Ctx), {llvm::PointerType::get(Type::getInt8Ty(Ctx),0), llvm::PointerType::get(Type::getInt8Ty(Ctx),0)}, false);
                    ...
                    if (GlobalVariable *GV = dyn_cast<GlobalVariable>(
                        ...
                        if (DecryptedGV.count(GV) > 0) {
                            ...
                            Instruction *InsertPoint =
                                PHI->getIncomingBlock(i)->getTerminator();
                            IRBuilder<> IRB(InsertPoint);
                            llvm::LLVMContext &Ctx = IRB.getContext();
                            Value *OutBuf = IRB.CreateBitCast(
                                Entry->DecGV, llvm::PointerType::get(Type::getInt8Ty(Ctx),0));
